<template>
    <section id="contact" aria-labelledby="contact">
        ...
        <div class="flex flex-col md:flex-row lg:gap-16 gap-8 justify-between md:px-0 px-4">
            <div class="basis-1/2 flex flex-col justify-between">
                <p class="text-lg font-medium tracking-tight text-slate-200 sm:text-xl">Tu as une question, une suggestion, ou juste envie de discuter ?</p>
                <p>Tu peux aussi m'envoyer un <a href="mailto:vincent@vinvui.com" class="text-yellow-500">email</a> si t'en as marre des formulaire ! 😉</p>
                <div class="flex justify-center">
                    <svg class="size-96 md:block hidden" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" viewBox="0 0 891.29496 745.19434" xmlns:xlink="http://www.w3.org/1999/xlink">
            ...
            <form @submit.prevent="sendForm" class="basis-1/2 grow flex flex-col gap-6">
                ...
                <div class="border-b border-slate-200 focus-within:border-yellow-400/40">
                    ...
                    <input type="email" v-model="form.email"
                        class="block bg-slate-900 w-full resize-none border-0 border-b border-transparent p-2 text-slate-100 placeholder:text-slate-500 focus:border-yellow-400/40 focus:ring-0 sm:text-sm sm:leading-6"
                        placeholder="Ton adresse email pour te répondre (promis j'en ferai rien d'autre 🫡)" required>
                </div>
                <div class="border-b border-slate-200 focus-within:border-yellow-400/40">
                    <label for="message"
                        class="leading-tight text-slate-200 text-lg font-medium tracking-tight sm:text-xl">
                    ...
                    <textarea id="message" v-model="form.message" rows="4"
                        class="block bg-slate-900 w-full resize-none border-0 border-b border-transparent p-2 text-slate-100 placeholder:text-slate-500 focus:border-yellow-400/40 focus:ring-0 sm:text-sm sm:leading-6"
                        placeholder="Qu'aimerais-tu me dire ?" required>
                    </textarea>
                </div>
                <!-- Honeypot field -->
                <div class="w-px h-px overflow-hidden">
                    <label for="honeypot" class="sr-only">Ne pas remplir ce champ</label>
                    <input type="text" id="honeypot" name="honeypot" v-model="form.honeypot" tabindex="-1" autocomplete="off">
                </div>
                <button type="submit"
                    class="bg-amber-400/50 hover:bg-amber-400/80 px-3 py-4 rounded-full text-slate-100 text-sm font-semibold uppercase tracking-wide whitespace-nowrap">
                    Envoie ton message
                </button>
            </form>
        </div>
    ...
    methods: {
        sendForm() {
            if (this.form.honeypot) {
                console.log("Honeypot field is filled. Probably a bot.");
                return;
            }
            console.log("Formulaire envoyé", this.form);
            // Logic to send the form to your server goes here
        }
    }
}
</script>
