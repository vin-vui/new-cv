                        ...
                        <TransitionChild as="template"
                            ...
                            <DialogPanel class="pointer-events-auto w-screen max-w-5xl">
                                <DialogTitle class="flex justify-between items-center px-6 py-4 bg-slate-900">
                                    ...
                                    <button @click="close" class="text-slate-200 hover:text-amber-400/80">
                                ...
                                <div class="flex h-full flex-col overflow-y-auto bg-slate-900 pb-6 shadow-xl">
                                    ...
                                    <div class="relative mt-6 flex-1 px-6">
                                        <div class="font-medium leading-snug text-slate-200">
                                            <div class="flex items-baseline gap-1 group">
                                                <a :href="project.external_url" v-if="project.external_url"
                                                    _target="blank" rel="noopener noreferrer"
                                                    class="text-lg text-slate-400 group-hover:text-amber-400/80">
                                                    <span>{{ project.client }}</span>
                                                    <ArrowUpRightIcon
                                                        class="size-4 inline-block transition-transform group-hover:-translate-y-1 group-hover:translate-x-1 motion-reduce:transition-none translate-y-px" />
                                                </a>
                                        ...
                                        <div class="mt-3 flex -ml-3">
                                            <div
                                                class="bg-amber-400/30 px-3 py-2 rounded-full text-slate-100 text-xs font-semibold uppercase tracking-wide whitespace-nowrap">
                                                {{ formatDate(project.start_date) }} — {{ formatDate(project.end_date)
                                                }}
                                            </div>
                                        </div>
                                        <div class="mt-6 text-base leading-normal text-slate-200">{{ project.description
                                            }}</div>
                                        <!-- External Links -->
                                                ...
                                                <tippy v-if="project.github" content="GitHub du projet">
                                                    <a :href="project.github" target="_blank"
                                                        ...
                                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                            class="h-8 w-8" aria-hidden="true" focusable="false">
                                            ...
                                            <div class="flex">
                                                <tippy v-if="project.url" content="Visiter le site web projet">
                                                    <a :href="project.url" target="_blank"
                                                        aria-label="Ouvrir l'URL du projet dans un nouvel onglet"
                                                        role="link" name="openProjectUrl" rel="noopener noreferrer"
                                                        class="text-slate-400 hover:text-slate-200 flex items-center gap-1">
                                    ...
                                    <div class="px-6 mt-8">
                                        ...
                                        <div class="grid md:grid-cols-4 sm:grid-cols-2 gap-2 mt-4">
                                            <div v-if="project.illustration_1"
                                                @click="showFullscreenImage(project.illustration_1)"
                                                class="cursor-pointer transition-all duration-200 hover:scale-105">
                                                <img :src="project.illustration_1" :alt="project.title"
                                                    class="w-full h-full object-contain">
                                            </div>
                                            <div v-if="project.illustration_2"
                                                @click="showFullscreenImage(project.illustration_2)"
                                                class="cursor-pointer transition-all duration-200 hover:scale-105">
                                                <img :src="project.illustration_2" :alt="project.title"
                                                    class="w-full h-full object-contain">
                                            </div>
                                            <div v-if="project.illustration_3"
                                                @click="showFullscreenImage(project.illustration_3)"
                                                class="cursor-pointer transition-all duration-200 hover:scale-105">
                                                <img :src="project.illustration_3" :alt="project.title"
                                                    class="w-full h-full object-contain">
                                            </div>
                                            <div v-if="project.illustration_4"
                                                @click="showFullscreenImage(project.illustration_4)"
                                                class="cursor-pointer transition-all duration-200 hover:scale-105">
                                                <img :src="project.illustration_4" :alt="project.title"
                                                    class="w-full h-full object-contain">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="md:px-6 mt-8 mb-12">
                                        ...
                                        <div class="mt-2">
                                            <ul
                                                class="list-none p-0 m-0 grid xl:grid-cols-6 sm:grid-cols-4 grid-cols-3">
                                                <li v-for="skill in project.skills" :key="skill.id"
                                                    ...
                                                    <a :href="skill.url" target="_blank" rel="noopener noreferrer"
                                                        ...
                                                        <div class="flex flex-col items-center gap-3">
                                                            <img :src="skill.img" :alt="skill.title + ' compétence'"
                                                                class="size-12">
                                                            <span class="font-semibold group-hover:text-amber-400/80">{{
                                                                skill.title }}</span>
                                                        </div>
                                                    </a>
                                                </li>
